...
ACTIFS = [
    ...
    "🇦🇺 AUD/USD 🇺🇸OTC", "🇪🇺 EUR/GBP 🇬🇧OTC", "🇺🇸 USD/PKR 🇵🇰OTC", "🇺🇸 USD/DZD 🇩🇿OTC",
...
def get_specific_or_random_image(directory: str, asset_filename: str = None):
    """
    Cherche une image correspondant au nom nettoyé de l'actif (ex: audcad_otc.jpg).
    Si elle n'existe pas, retourne une image aléatoire du dossier.
    """
    ...
    # 1. Recherche de l'image spécifique
    if asset_filename:
    ...
    # 2. Fallback : image aléatoire dans le dossier
    images = [
...
def format_signal_text(signal: dict) -> str:
    """Formate le texte du signal selon les exigences exactes."""
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Répond à /start en affichant uniquement le bouton AVOIR SIGNAL."""
...
async def send_signal_action(query_or_update, context: ContextTypes.DEFAULT_TYPE):
    """Génère et envoie un signal avec son image et les boutons de résultat."""
    signal = generate_signal_data()
    message_text = format_signal_text(signal)

    # Sauvegarde du nom nettoyé de la paire envoyée
    asset_key = get_asset_filename(signal['actif'])
    context.user_data['last_asset'] = asset_key

    image_path = get_specific_or_random_image(DIR_IMG, asset_key)
...
async def handle_button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Gère l'ensemble des clics sur les boutons Inline."""
    ...
    # 2. Clic sur un bouton de résultat
    results_map = {
        "res_mg0": ("✅GAIN DIRECT✅", DIR_WIN),
        "res_mg1": ("✅GAIN MARTINGALE 1✅", DIR_WIN),
        "res_mg2": ("✅GAIN MARTINGALE 2✅", DIR_WIN),
        "res_mg3": ("✅GAIN MARTINGALE 3✅", DIR_WIN),
        "res_lose": ("❌PERDU❌", DIR_LOSE)
    }

    if data in results_map:
        caption_text, target_dir = results_map[data]

        # Récupération de l'actif associé au dernier signal
        last_asset = context.user_data.get('last_asset', None)

        # Sélection de l'image (spécifique à la paire dans DIR_WIN si disponible)
        image_path = get_specific_or_random_image(target_dir, last_asset)
